...
def check_eligibility(alert_card, inst_row):
    ...
    # Get assets - try multiple sources
    assets = 0
    if 'ASSET' in inst_row:
        assets = float(inst_row.get('ASSET', 0) or 0) * 1000
        print(f"DEBUG: Assets from inst_row ASSET: {assets}")
    elif 'assets' in alert_card:
        assets = float(alert_card.get('assets', 0) or 0)
        print(f"DEBUG: Assets from alert_card assets: {assets}")
    elif 'ASSET' in alert_card:
        assets = float(alert_card.get('ASSET', 0) or 0) * 1000
        print(f"DEBUG: Assets from alert_card ASSET: {assets}")
    else:
        assets = 100000000  # Default $100M for demo
        print(f"DEBUG: Using default assets: {assets}")
    
    # Get membership flags - default to 0 if missing
    ...
    for prog in eligible[:2]:
        try:
            ...
            try:
                ...
                print(f"    API Error for {prog['name']}: {api_error}")
                # Generate fallback brief when API fails
                brief = f"""**EMERGENCY LIQUIDITY REQUEST - {prog['name']}**

**Bank:** {name}
**Situation:** Risk score {alert_dict.get('RISK_SCORE', alert_dict.get('risk_score', 0)):.1f}/100 indicates severe liquidity stress.

**Program:** {prog['name']}
**Estimated Access:** ${prog['estimated_amount']:,.0f}
**Processing Time:** {prog['speed_days']} business days

**Recommendation:** Immediate application to {prog['name']} for emergency liquidity support.

**Contact:** {prog.get('contact', 'Regional Federal Reserve Bank')}
**Application:** {prog.get('application_url', 'Contact your regional Fed')}

*This brief was generated by the MDI Liquidity Monitor AI system.*"""
